...
`sftraj` from two developers of key packages (`amt` and
`crawl`). Altogether, feedback received only confirms the need for a
central trajectory package in R.
...
* **Submission to CRAN** [month 1]: During the first stage of the
  ...
  allow for easy installation on any computer, which is mandatory for
  wide adoption.
* **Submission to rOpenSci** [month 2]: After the package has been
  ...
  fit in the special category "geospatial data". Becoming an official
  rOpenSci package would give the package an extra exposure, and
  guarantee its overall usefulness and usability.
...
with more detailed blog posts on https://mablab.org/. We will
constantly watch for feedback, notably through the GitHub repository,
which will remain constantly open.
